...
func (rt *Runtime) TableStateUpdated(p psae.PSAE, ts *psae.TableState) {
...
func (rt *Runtime) PlayerJoined(p psae.PSAE, player *psae.Player) {
}

func (rt *Runtime) PlayerDispatched(p psae.PSAE, player *psae.Player) {
...
func (rt *Runtime) PlayerReleased(p psae.PSAE, player *psae.Player) {
	...
	//fmt.Printf("Found new table %s (player_count=%d, avail=%d) for player (%s)\n", target.ID, len(target.psae.Players), target.AvailableSeats, player.ID)
...
func (rt *Runtime) Matched(p psae.PSAE, m *psae.Match) {
...
func (rt *Runtime) WaitingRoomDrained(p psae.PSAE, player *psae.Player) {
...
func (rt *Runtime) WaitingRoomEntered(p psae.PSAE, player *psae.Player) {
}

func (rt *Runtime) WaitingRoomLeft(p psae.PSAE, player *psae.Player) {
}

func (rt *Runtime) WaitingRoomMatched(p psae.PSAE, players []*psae.Player) {
